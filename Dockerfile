...
# && sed -i 's|NCARG_ROOT}/lib|NCARG_ROOT}|g' ./configure
RUN sed -i '349s|-L${NETCDF}/lib -lnetcdf $NETCDFF|-L${NETCDF}/lib $NETCDFF -lnetcdff -lnetcdf -lnetcdf -lnetcdff_C -lhdf5 |g' ./configure \
...
RUN source /root/miniconda3/etc/profile.d/conda.sh && conda activate ncl_stable && ./compile 
#> log_compile.txt
